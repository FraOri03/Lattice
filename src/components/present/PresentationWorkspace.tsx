...
function elementStyle(el: PresentElement): React.CSSProperties {
  return {
    position: 'absolute',
    left: el.x,
    top: el.y,
...
function StaticElement({ el, themeText }: { el: PresentElement; themeText: string }) {
  if (el.kind === 'text') {
    return (
      <div
        style={{
  ...
  if (el.kind === 'image') {
    return <img src={el.src} alt="" style={{ ...elementStyle(el), objectFit: 'fill' }} draggable={false} />
  }
...
/** Read-only mini render used for slide-list thumbnails. */
