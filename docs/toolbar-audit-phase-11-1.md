...
| Item | Why | Tracked |
|---|---|---|
| ~~Board overflow below ~1060 px~~ | **Done in 12.4.** A folded split button becomes *its items, flattened into the menu* — the alternative (one entry that repeats the last tool) keeps the convenience and loses the other six tools, and "repeat the last tool" means nothing on a control you already had to open a menu to reach. Fold order, deliberate: Media first, then Create, and Section last; the Comment toggle never folds. Wiring it also found a bug in the primitive — it summed the foldable items and ignored the bar's own padding, separators and unfoldable controls, so 252 px of tools in a 361 px bar read as fitting a 340 px pane while the bar was clipped 11 px each side. | [#47](https://github.com/FraOri03/Lattice/issues/47) |
| ~~`.tbtn` on the bubble/block menus and the slide inspector~~ | **Done in 12.4/12.5.** All three surfaces are on the primitives; the class is deleted. A second, *unlayered* copy of `.tbtn` had survived the move into `@layer components`, so the per-instance utilities that move was meant to unblock were still losing to it. | [#48](https://github.com/FraOri03/Lattice/issues/48) |
