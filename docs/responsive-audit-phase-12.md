...
| **12.4** | Mode toolbars fold: wire `useToolbarOverflow` on the Board. **Shipped.** Fold order is Media, then Create, with Section last and the Comment toggle never folding; a folded split button becomes its items, flattened. The primitive gained the pure `fitCount` (so the boundary is testable without a layout), an explicit bounds ref (a `w-max` pill measured against itself always fits) and a fix: it had been ignoring the bar's own padding and unfoldable controls. Only the board's toolbar clipped: it floats over the canvas as a fixed-width pill, while `.doc-toolbar` wraps and `.sheet-toolbar` wraps and then scrolls rather than clipping. The others were left alone rather than folded for symmetry. | F5, [#47](https://github.com/FraOri03/Lattice/issues/47) |
| **12.5** | The Viewer tier: honest panels for Sheet/Code/Presentation/Photo, the board navigable but not layoutable, comments reachable everywhere. **Shipped.** The `.tbtn` migration covered the rich-document bubble and block menus — the surface that matters here, since documents stay editable at this tier. The slide element inspector's six remain in [#48](https://github.com/FraOri03/Lattice/issues/48): it sits behind the desktop-only panel at this tier, so nothing in the Viewer story depends on it. | F6, part of [#48](https://github.com/FraOri03/Lattice/issues/48) |
